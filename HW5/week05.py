...
def make_graph(n, density, max_w, seed):
...
def run_experiment(ns, trials, density, max_w):
    results = []
    ...
    for n in ns:
        ...
        for t in range(trials):
            ...
            mst1 = MST(G)
            Tb = mst1.boruvka()
            w1 = mst_weight(Tb, G[0][0])

            mst2 = MST(G)
            Tk = mst2.kruskal()
            w2 = mst_weight(Tk, G[0][0])
            ...
            bor_times.append(end - start)

            start = time.perf_counter()
            MST(G).kruskal()
            end = time.perf_counter()
            kru_times.append(end - start)

        results.append({
            "n": n,
            "trials": trials,
            "avg_boruvka": mean(bor_times),
            "std_boruvka": std_dev(bor_times),
            "avg_kruskal": mean(kru_times),
            "std_kruskal": std_dev(kru_times),
        })
...
def print_table(results):
    print("n\ttrials\tavg_boruvka\tstd_boruvka\tavg_kruskal\tstd_kruskal")
    for r in results:
        print(
            str(r["n"]) + "\t" +
            str(r["trials"]) + "\t" +
...
if __name__ == "__main__":
    ...
    Tb = MST(graph).boruvka()
    Tk = MST(graph).kruskal()
    print(Tb)
    print(Tk)
    print("Boruvka weight:", mst_weight(Tb, 0))
    print("Kruskal weight:", mst_weight(Tk, 0))
    ...
    results = run_experiment(ns, trials, density, max_w)
    print_table(results)